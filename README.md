...
- **Sentinel SRE**: Autonomous health monitoring and **Self-Healing Loop** that audits trading strategy and automatically generates technical fixes.
    ...
    - **Manual Run**:
        ...
        ```
    - **Self-Healing**: Integrated `HealerAgent` uses **Vertex AI (Gemini)** to translate strategic findings into technical directives, which can be applied by **Antigravity AI Agent** to fix gating issues, weight adjustments, and threshold tuning.
    - **LLM-Powered Analysis**: Both Sentinel (strategic audit) and Healer (directive generation) leverage Vertex AI for intelligent code-level recommendations.
